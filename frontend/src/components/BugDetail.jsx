...
// ✅ Developers can only mark a bug as Resolved
const DEVELOPER_STATUS_OPTIONS = ["Resolved"];
...
const BugDetail = () => {
  ...
  // Derived flag — true when the logged-in user is a Developer
  ...
  // ✅ Developer inline status dropdown state
  ...
  const fetchBug = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // ✅ Developer dropdown always starts at Resolved (their only option)
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (!assignForm.assigned_to && response.data.developers.length > 0) {
          setAssignForm((f) => ({
            ...f,
            assigned_to: response.data.developers[0]._id,
          }));
        }
      }
    } catch (err) {
  ...
  // ✅ Developer inline status update handler
  const handleDeveloperStatusUpdate = async () => {
    if (selectedStatus === bug.status) return;
    setActionError("");
    setStatusUpdateLoading(true);
    setStatusSuccess(false);
    try {
      const payload = { status: selectedStatus };
      const response = await bugAPI.update(id, payload);
      if (response.data.success) {
  ...
  // --- Full edit submit (non-developer roles) ---
  ...
  // --- Status update via action buttons ---
          ...
          {!editing ? (
            <>
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <span
                      className={`px-2 py-1 rounded text-sm font-medium ${STATUS_COLORS[bug.status] || "bg-gray-100 text-gray-700"}`}
                    >
                      {bug.status}
                    </span>
                    <span
                      className={`px-2 py-1 rounded text-sm font-medium ${PRIORITY_COLORS[bug.priority] || "bg-gray-100 text-gray-700"}`}
                    >
                      {bug.priority} Priority
                    </span>
                  </div>
                </div>
                {/* ✅ Edit button only for non-developers */}
                {access.canEdit && !isDeveloper && (
                  <button
              ...
              <div className="space-y-4 text-sm text-gray-700">
                ...
                {/* ✅ Inline Update Status section — developers only */}
                {isDeveloper && access.canEdit && (
                  <div className="pt-4 border-t">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {DEVELOPER_STATUS_OPTIONS.map((s) => (
                      ...
                      <button
                        onClick={handleDeveloperStatusUpdate}
                        disabled={
                          statusUpdateLoading || selectedStatus === bug.status
                        }
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {access.canAssign && (
                <button
                  onClick={
                    assigning ? () => setAssigning(false) : handleOpenAssign
                  }
            ...
            {confirmDelete && (
              <div className="mt-4 p-4 bg-red-50 rounded border border-red-200 space-y-3">
                <p className="text-sm text-red-700 font-medium">
                  Are you sure you want to delete this bug? This cannot be
                  undone.
                </p>
