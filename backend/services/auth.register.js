...
export async function registerUser(userData, res) {
  try {
    ...
    if (!name || !email || !password || !role) {
      return { success: false, statusCode: 400, message: 'All fields are required: name, email, password, role' };
    }

    const existingUser = await User.findOne({ email });
    if (existingUser) {
      return { success: false, statusCode: 400, message: 'User already exists with this email' };
    }

    const selectedRole = await Role.findById(role);

    if (!selectedRole) {
      return { success: false, statusCode: 400, message: 'Invalid role ID' };
    }
    ...
    const permissions = rolePermissions.map(rp => rp.permission?.name).filter(Boolean);
    ...
    return { success: true, statusCode: 201, message: 'Registration successful!', data: userData_response };

  } catch (error) {
    console.error('Registration error:', error);
    return { success: false, statusCode: 500, message: 'Registration failed', error: error.message };
  }
}